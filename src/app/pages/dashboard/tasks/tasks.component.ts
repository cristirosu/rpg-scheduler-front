...
export class TasksComponent implements OnInit {
  ...
  constructor(
    ...
    this.calendarConfiguration.eventClick = (calEvent, jsEvent, view) => {
      for (let i = 0; i < this.tasks.length; i++) {
        if (this.tasks[i].id == calEvent.id) {
          this.openSaveTaskWindow(this.tasks[i]);
          break;
        }
      }
    }
  ...
  private _onSelect(start, end): void {
    let task: Task = new Task(undefined, moment(start).format('YYYY-MM-DD HH:mm:ss.s'));
    this.openSaveTaskWindow(task);
  }
